...
class LCNet(nn.Module):
    ...
    def prepareInputs(self, x):
        ...
        if (h == t_h and w == t_w):
            ...
            imgs = torch.nn.functional.interpolate(x[0], size=(t_h, t_w), mode='bilinear', align_corners=True)
        ...
        if self.other['in_mask']:
            ...
            if mask.shape[2] != inputs[0].shape[2] or mask.shape[3] != inputs[0].shape[3]:
                mask = torch.nn.functional.interpolate(mask, size=(t_h, t_w), mode='bilinear', align_corners=True)
            for i in range(len(inputs)):
    ...
    def forward(self, x):
        ...
        if self.other['s1_est_d']:
            ...
            pred['dirs_x'] = torch.cat(l_dirs_x, 0).reshape(len(l_dirs_x),-1)
            pred['dirs_y'] = torch.cat(l_dirs_y, 0).reshape(len(l_dirs_y),-1)
