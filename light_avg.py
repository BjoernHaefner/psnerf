...
for vi in range(n_view):
    ...
    for idx, li in enumerate(light_slt[vi]):
        ...
        if args.light_intnorm:
            ...
            print(f"Save: {realpath(os.path.join(normalizedir, 'view_{:02d}/{:03d}.png'.format(vi+1,li+1)))}")
    ...
    print(f"Save: {realpath(os.path.join(normalizedir, avgdir,'view_{:02d}.png'.format(vi+1)))}")
