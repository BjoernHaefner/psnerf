...
class Renderer(nn.Module):
    ...
    def forward(self, pixels, camera_mat, world_mat, scale_mat, 
                      ...
                      it=0, visibility=False, light_dir=None):
        ...
        elif rendering_technique == 'shape_extract':
            out_dict = self.shape_extract(
                pixels, camera_mat, world_mat, 
                scale_mat, it=it,
                visibility=visibility, light_dir=light_dir,
            )
        else:
            print("Choose unisurf, phong_renderer or shape_extract")
        return out_dict
    ...
    def unisurf(self, pixels, camera_mat, world_mat, 
        ...
        norm_pred = torch.zeros_like(rgb_values)
        norm_pred[surface_mask] = normals_[:N]
        diff_norm =  torch.norm(normals_[:N] - normals_[N:], dim=-1) if not eval_ else None
    ...
    def phong_renderer(self, pixels, camera_mat, world_mat, 
        ...
        light = (light_source / light_source.norm(2)).unsqueeze(1).to(device)
        ...
        diffuse = torch.mm(surface_normals, light).clamp_min(0).repeat(1, 3) * diffuse_per.unsqueeze(0).to(device)
        rgb_values[network_object_mask] = (ambiant.unsqueeze(0).to(device) + diffuse).clamp_max(1.0)
    ...
    def shape_extract(self, pixels, camera_mat, world_mat, 
                scale_mat, it=100000, 
                visibility=False, light_dir=None):
        # Get configs
        batch_size, n_points, _ = pixels.shape
        device = self._device
        rad = self.cfg['radius']
        ...
        with torch.no_grad():
            ...
            # Derive Normals
            norm_pred = torch.zeros(batch_size * n_points, 3, device=device)
            if len(surface_points) > 0:
                grad = []
                for pnts in torch.split(surface_points, 1000000, dim=0):
                    grad.append(self.model.gradient(pnts, tflag=False)[:,0,:].detach())
                    torch.cuda.empty_cache()
                grad = torch.cat(grad,0)
                norm_pred[network_object_mask] = F.normalize(grad, dim=-1)

        out_dict = {
            'mask': network_object_mask.reshape(batch_size, -1, ),
            'normal': norm_pred.reshape(batch_size, -1, 3),
            'points' : points.reshape(batch_size, -1, 3),
        }

        if visibility and light_dir is not None:
            vis_map = torch.ones(light_dir.shape[0], n_points, device=device)
            if len(surface_points) > 0:
                with torch.no_grad():
                    lbatch = 96
                    lall = []
                    for lstart in range(0,len(light_dir), lbatch):
                        lend = min(len(light_dir),lstart+lbatch)
                        lall.append(self.light_visibility(surf=surface_points, light_dir=light_dir[lstart:lend]))
                    vis_map[network_object_mask[None,].expand_as(vis_map)] = torch.cat(lall, dim=0)
            out_dict['visibility'] = vis_map
            torch.cuda.empty_cache()
    ...
    def light_visibility(self, surf=None, light_dir=None,
                        lnear=0.1, lfar=3.5,
                        tau=0.5, n_steps=128, max_points=3500000,
                        ):
        # near = 0.1 for removing points near the surface
        rays_o = surf[None,].expand(light_dir.shape[0],-1,-1)
        rays_d = light_dir[:,None].expand_as(rays_o)
        device = surf.device
        # [L,3] -> [L,1,3] -> [L,N,3] light direction
        # [N,3] -> [1,N,3] -> [L,N,3] surface points

        nl, nr, _ = rays_d.shape # [L,N,3]

        d_proposal = torch.linspace(0, 1, steps=n_steps).view(1, 1, n_steps, 1).to(device)
        d_proposal = lnear * (1. - d_proposal) + lfar* d_proposal
        p_proposal = rays_o.unsqueeze(2).repeat(1, 1, n_steps, 1) + \
                        rays_d.unsqueeze(2).repeat(1, 1, n_steps, 1) * d_proposal
        # [L,N,S,3]
        with torch.no_grad():
            alpha = torch.cat([
            ...
            # ignore density out of object box [-1,1]
            amask = torch.logical_and((p_proposal<=1.1).all(dim=-1), (p_proposal>=-1.1).all(dim=-1)).view(-1, n_steps)
            alpha[~amask]=0
            weights = alpha * torch.cumprod(torch.cat([torch.ones((alpha.shape[0], 1), device=device), 1.-alpha + epsilon ], -1), -1)[:, :-1]
            acc_map = torch.sum(weights, -1)#.view(nl,nr)
